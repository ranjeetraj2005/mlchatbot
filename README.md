...
- [ ] Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv mlchatbot` or `python -m venv mlchatbot-venv`.
...
- [ ] Make sure the virtualenv is activated `workon mlchatbot` or `source mlchatbot-venv/bin/activate`.
- [ ] Compile the requirements before installation and install them:  `make compile_install_requirements`
- [ ] Change the first line of README to the name of the project
- [ ] Add an email address to the `ADMINS` settings variable in `mlchatbot/backend/mlchatbot/settings/base.py`
- [ ] Change the `SERVER_EMAIL` to the email address used to send e-mails in `mlchatbot/backend/mlchatbot/settings/production.py`
...
- Create a copy of ``mlchatbot/settings/local.py.example``:  
  `cp mlchatbot/settings/local.py.example mlchatbot/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/mlchatbot/settings/base.py``.
- Adjust ``backend/mlchatbot/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
